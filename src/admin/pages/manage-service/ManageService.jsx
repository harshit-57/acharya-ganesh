...
import PaginationTable from './components/ServiceTable';
...
const ManageService = () => {
    ...
    const [sortBy, setSortBy] = useState('sr."PublishedOn"');
    ...
    const fetchServices = async () => {
        try {
            ...
            if (searchQuery && searchQuery?.length >= 3) {
                response = await APIHelper.getServices({
                    page: currentPage,
                    pageSize: pageSize,
                    search: searchQuery,
    ...
    const exportData = async () => {
        ...
        try {
            const res = await APIHelper.getServices({
                page: 1,
                pageSize: totalCount,
            });
            ...
            PrintExcel(
                res?.data?.data,
                `ServiceData-${moment(new Date()).format('YYYY-MM-DD')}.xlsx`
            );
        ...
        <Fragment>
                        ...
                        >
                            <Box sx={{ display: 'flex', gap: '15px' }}>
                                <Button
                                    ...
                                    onClick={() =>
                                        navigate(
                                            '/admin/content-editor/service/new',
                                            {
                                                state: {},
                                            }
                                        )
            ...
            {showDeleteAlert && (
                <AlertDialog
                    ...
                    description="Are you want to sure delete the service?"
